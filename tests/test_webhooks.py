...
async def test_signed_delivery_succeeds(tmp_path: Path, monkeypatch):
    monkeypatch.setattr(socket, "getaddrinfo", public_dns)
    store = WebhookStore(tmp_path / "w.db"); store.subscribe("u", "https://hooks.example/mee", {"job.done"}); store.enqueue("e", "job.done", {"ok":True})
    seen = {}
    def handler(request):
...
async def test_failed_delivery_requeues(tmp_path: Path, monkeypatch):
    monkeypatch.setattr(socket, "getaddrinfo", public_dns)
    store = WebhookStore(tmp_path / "w.db"); store.subscribe("u", "https://hooks.example/mee", {"*"}); store.enqueue("e", "job.failed", {})
