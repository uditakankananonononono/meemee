...
- PostgreSQL operator tools (`meemee_persist_pg.operator_tools`, no migration): `account-export`, `account-import`, `retention-run`, `audit-anchor`, `audit-anchor-verify` and `audit-prune` work in PostgreSQL mode on the shared database instead of refusing; same export format on both backends; one-transaction import; pruning under the audit append lock. Without `MEEMEE_POSTGRES_DSN` they stop before touching local files. Tests: `tests_pg/test_operator_tools_pg.py`, `tests/test_operator_tools_multihost_e2e.py`.
- Shared model layer: vendored `instinct_models` (shared-models @ pin), `shared` profile (transport `instinct`, Needle -> Ornith -> Inkling, private by default), `MEEMEE_SHARED_*` settings, Meemee-only Needle LoRA training (`meemee.shared_models.train_meemee_needle`).
- PostgreSQL-backed secret vault (migration 015, `meemee_persist_pg.SecretVault`, `meemee.vault.vault_from_settings`) and PostgreSQL key rotation (`rotate_pg_keys`): `rotate-encryption-key` in PostgreSQL mode re-encrypts the shared vault and webhook subscription secrets atomically. Cutover group `--vault`. docs/PG.md documents sticky browser routing as a deployment requirement.
- PostgreSQL-backed account-deletion ledger, browser session records and takeover notices (migrations 013, 014): `meemee_persist_pg.DeletionLedger`, `BrowserSessionStore`, `TakeoverNoticeQueue`; `Persistence.deletion_ledger`/`browser_sessions`/`browser_notices`. Browser sessions record `host_id`; a restart marks only that host's sessions lost; requests reaching another host fail naming the owning host. Cutover groups `--deletions`, `--browser`, `--notices`. docs/PG.md per-host list corrected (it still listed the stores moved in groups 6 and 7).
- PostgreSQL-backed monitors and reflection schedule: `meemee_persist_pg.MonitorStore`/`ReflectionSchedule`, migration `012_monitors_reflection.sql`, `Persistence.monitors`/`reflection_schedule`; wired into the API, readiness, account deletion and `meemee reflection-worker`. Cutover groups `--monitors` and `--reflection`.
...
## Unreleased (branch pg-companion)

- `meemee_persist_pg.CompanionStore` (migration `010_companion.sql`), same contract as SQLite plus `ping`; `build_persistence` returns `companion`, the API passes it to the companion layer, and `build_companion` uses the shared store in PostgreSQL mode for the companion worker and CLI. Readiness and account deletion use it.
- Cutover: optional `companion` group.
- `CompanionStoreInterface` added.
...
- SECURITY: job webhooks leaked across tenants. `WebhookStore.enqueue` matched every active subscription of every principal, so each `job.done` (with the full run result), `job.failed` and `job.cancelled` event was also delivered to other principals' endpoints. `enqueue` now requires the owning `principal` and matches only that principal's subscriptions; the worker passes the job owner and the cancel endpoint the caller. Pre-release, no known deployed multi-tenant instance, but any instance with webhooks from more than one principal should treat past job-event deliveries as exposed. Proven by `tests/test_webhook_private_override.py` and a live second-principal-gets-nothing test (fails on the old code).
- Added test-only `MEEMEE_WEBHOOK_ALLOW_PRIVATE_HOSTS=1`: webhook URLs may resolve to loopback/private hosts (HTTPS still required) so a local receiver can be tested. Refused when `MEEMEE_ENV=production`: `validate_webhook_url` raises and the API and `webhook-worker` exit at startup; otherwise they log a warning. New `MEEMEE_ENV` setting (default `development`).
- Added a live webhook test to `tests/test_live_runs_e2e.py` (SQLite and PostgreSQL job stores): a real `webhook-worker` delivers `job.done` over TLS to a local HTTPS receiver; payload carries `blocked` and `approvals_required`; `X-Meemee-Signature-256` verifies with `meemee.webhook_verify.verify_signature` and fails for a changed body or wrong secret; a 500 is retried with the same delivery ID after backoff and the attempt timeline records failure then success.
