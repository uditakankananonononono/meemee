...
- In PostgreSQL mode, runs, idempotency keys, webhooks and the companion/personal-model stores are still per-host SQLite files; memory, jobs, rate limits, tool-approval grants, API tokens, accounts, the audit chain, email-verification/password-reset challenges, daily quotas and plan assignments are shared through PostgreSQL. Runs listed on one host are not visible on another. `meemee account-export`/`account-import` refuse to run in PostgreSQL mode.
