...
- In PostgreSQL mode, every product store listed in docs/PG.md is shared. Still per-host: browser profiles (`browser-profiles/`) and the live browser process (takeover links and browser requests must reach the host that opened the session; other hosts answer with its host id), and `operations.sqlite3` (local migration bookkeeping). Sticky routing of browser and takeover requests to the owning host is a deployment requirement (docs/PG.md). The secret vault is shared and `meemee rotate-encryption-key` re-encrypts the PostgreSQL vault and webhook secrets atomically; memory, jobs, rate limits, tool-approval grants, API tokens, accounts, the audit chain, email-verification/password-reset challenges, daily quotas, plan assignments, run history, idempotency keys, webhook subscriptions/deliveries, companion state, the personal model, connected context, monitors, reflection watermarks, the account-deletion ledger, browser session records and takeover notices are shared through PostgreSQL. `meemee account-export`/`account-import`, `retention-run` and the audit-anchor commands act on the shared database in PostgreSQL mode (item 187).
- Audit anchors, `meemee audit-anchor-verify` and `meemee audit-prune` work on both backends; in PostgreSQL mode they sign, verify and prune the one shared chain, and pruning holds the append lock (item 187).
