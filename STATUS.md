...
**Current core version:** 0.67.0  
**Ledger:** 91 verified, 0 thin  
**Supported production shape:** one host, one or more API/worker processes, SQLite/WAL on local persistent storage  
**Last core verification:** 191 passed; 112 SDK passed; Ruff and explicit 0.67.0 release audit clean

## What is production-usable now

The authoritative detailed list is the 91-item "Verified" section in [README.md](README.md). The main product surfaces are:
...
|---|---|---|
| Core server/runtime | `pytest -q` after v0.67.0 | 191 passed |
| Core lint | `ruff check meemee tests examples/webhook_receiver` after v0.67.0 | clean |
| SDK | v0.67.0 full run | 112 passed, including 11 against a booted server |
