...
- In PostgreSQL mode, every product store listed in docs/PG.md is shared. Still per-host: the secret vault (`vault.sqlite3`, used by `meemee vault-*` and vault-backed tools), browser profiles (`browser-profiles/`) and the live browser process (takeover links and browser requests must reach the host that opened the session; other hosts answer with its host id), and `operations.sqlite3` (local migration bookkeeping). `meemee rotate-encryption-key` re-encrypts the local `vault.sqlite3`/`webhooks.sqlite3` only, not PostgreSQL webhook secrets; memory, jobs, rate limits, tool-approval grants, API tokens, accounts, the audit chain, email-verification/password-reset challenges, daily quotas, plan assignments, run history, idempotency keys, webhook subscriptions/deliveries, companion state, the personal model, connected context, monitors, reflection watermarks, the account-deletion ledger, browser session records and takeover notices are shared through PostgreSQL. `meemee retention-run` prunes local SQLite files only and refuses to run in PostgreSQL mode. `meemee account-export`/`account-import` refuse to run in PostgreSQL mode.
