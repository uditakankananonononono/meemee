...
- In PostgreSQL mode, these stores are still per-host SQLite files in each host's data directory: monitors (`monitors.sqlite3`), reflection schedule, account-deletion ledger, browser sessions and takeover notices. Run the features that use them (monitors, the reflection schedule, browser takeover) on one host; memory, jobs, rate limits, tool-approval grants, API tokens, accounts, the audit chain, email-verification/password-reset challenges, daily quotas, plan assignments, run history, idempotency keys, webhook subscriptions/deliveries, companion state, the personal model and connected context are shared through PostgreSQL. `meemee retention-run` prunes local SQLite files only and refuses to run in PostgreSQL mode. `meemee account-export`/`account-import` refuse to run in PostgreSQL mode.
