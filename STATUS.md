...
- In PostgreSQL mode, the companion/personal-model stores are still per-host SQLite files; memory, jobs, rate limits, tool-approval grants, API tokens, accounts, the audit chain, email-verification/password-reset challenges, daily quotas, plan assignments, run history, idempotency keys and webhook subscriptions/deliveries are shared through PostgreSQL. `meemee retention-run` prunes local SQLite files only and refuses to run in PostgreSQL mode. `meemee account-export`/`account-import` refuse to run in PostgreSQL mode.
