...
- In PostgreSQL mode, runs, idempotency keys, quotas, entitlements, webhooks and the companion/personal-model stores are still per-host SQLite files; memory, jobs, rate limits, tool-approval grants, API tokens, accounts, the audit chain and email-verification/password-reset challenges are shared through PostgreSQL. Runs listed on one host are not visible on another, and each host counts daily quotas separately.
