...
**Current core version:** 0.65.0  
**Ledger:** 89 verified, 0 thin  
**Supported production shape:** one host, one or more API/worker processes, SQLite/WAL on local persistent storage  
**Last core verification:** 190 passed; 110 SDK passed; Ruff and explicit 0.65.0 release audit clean

## What is production-usable now

The authoritative detailed list is the 89-item "Verified" section in [README.md](README.md). The main product surfaces are:
...
|---|---|---|
| Core server/runtime | `pytest -q` after v0.65.0 | 190 passed |
| Core lint | `ruff check meemee tests examples/webhook_receiver` after v0.65.0 | clean |
