...
**Current core version:** 0.41.0  
**Ledger:** 64 verified, 0 thin  
**Supported production shape:** one host, one or more API/worker processes, SQLite/WAL on local persistent storage  
**Last core verification:** 135 core + 109 SDK passed; core/SDK scoped Ruff and release audit clean

## What is production-usable now

The authoritative detailed list is the 64-item "Verified" section in [README.md](README.md). The main product surfaces are:
...
|---|---|---|
| Core server/runtime | `pytest -q` after v0.41.0 | 135 passed |
| Core lint | `ruff check meemee tests examples/webhook_receiver` after v0.41.0 | clean |
| SDK | v0.41.0 full run | 109 passed, including 11 against a booted server |
