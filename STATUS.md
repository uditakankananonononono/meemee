...
**Current core version:** 0.38.0  
**Ledger:** 60 verified, 0 thin  
**Supported production shape:** one host, one or more API/worker processes, SQLite/WAL on local persistent storage  
**Last core verification:** 128 passed, Ruff clean; release audit clean

## What is production-usable now

The authoritative detailed list is the 60-item "Verified" section in [README.md](README.md). The main product surfaces are:
...
|---|---|---|
| Core server/runtime | `pytest -q` after v0.38.0 | 128 passed |
| Core lint | `ruff check meemee tests examples/webhook_receiver` after v0.38.0 | clean |
