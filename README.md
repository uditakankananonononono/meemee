...
### OpenClaw / Hermes (free local route)
Set `MEEMEE_SHARED_HERMES_URL=http://127.0.0.1:11434/v1` and `MEEMEE_SHARED_HERMES_MODEL=hermes3:3b`; pull the free Hermes 3 weights via local Ollama first and choose Meemee's `shared` model profile. Meemee's existing shared chain tries Hermes locally after Ornith/Inkling local, with hosted fallback off by default. `meemee.shared_models.openclaw_owner_call` is an explicit, authenticated-owner-only call to a separately installed local OpenClaw Gateway, never a model-chain fallback: set `MEEMEE_OPENCLAW_URL=http://127.0.0.1:18789/v1` and `MEEMEE_OPENCLAW_TOKEN` only on a trusted single-owner host, enable its chat-completions endpoint, and pass `owner_confirmed=True` only after owner authentication. Never expose it as a public API: the gateway bearer grants full operator rights. Hermes Agent is a separate program, not installed or falsely claimed by this Hermes model route. No paid keys, card or hosted route is needed; a running local Ollama, model download, and (for OpenClaw) separately installed/configured gateway remain necessary. Details and sources: https://github.com/uditakankananonononono/shared-models ; https://docs.openclaw.ai/gateway/openai-http-api ; https://ollama.com/library/hermes3 ; https://hermes-agent.nousresearch.com/docs/guides/local-ollama-setup .
