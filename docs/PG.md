...
This package provides PostgreSQL implementations of memory, jobs, plans, API tokens, audit, migrations, rate limits and tool-approval grants. SQLite remains the default. `MEEMEE_PERSISTENCE_BACKEND=postgresql` with `MEEMEE_POSTGRES_DSN` makes the API and `meemee worker` use PostgreSQL for memory, jobs, rate limits, tool-approval grants, API tokens, customer accounts, the audit chain, email-verification/password-reset challenges, daily job quotas, plan assignments, run history and idempotency keys (other product stores are still per-host SQLite; see STATUS).
