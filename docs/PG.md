...
## Automated offline copy and dual verification

Stop all Meemee API/worker writers first. The copy refuses non-empty PostgreSQL target tables, holds consistent read transactions across every SQLite source, checks SQLite `data_version` before PostgreSQL commit, and rolls the entire target transaction back if another SQLite connection committed during the copy.

```bash
export MEEMEE_POSTGRES_DSN='postgresql://...'
python -m meemee_persist_pg.cli \
  --memory data/meemee.sqlite3 --jobs data/jobs.sqlite3 \
  --plans data/plans.sqlite3 --tokens data/auth.sqlite3 \
  --audit data/audit.sqlite3 copy > cutover-report.json
```

The command runs SQLite integrity/schema checks, applies migrations, copies in dependency order, resets identity sequences, and compares each table using both row count and a length-delimited SHA-256 over canonical rows. It exits 2 on any mismatch. `verify` reruns one comparison; `dual-verify --cycles 6 --interval-seconds 10` repeats comparisons for an operator-selected observation window. This is verification tooling, not online dual-write: keep writers stopped throughout copy and verification.
